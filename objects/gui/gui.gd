...
const HEART_INCREASE_RATE : float = 24
...
func _ready() -> void:
	...
	heart_count = %Systems.player_properties.hp * HEART_ANIMATION_FRAME_COUNT
	gold_count = %Systems.player_properties.gold
...
func _process(delta: float) -> void:
	...
	for heart_container in heart_sprites:
		...
		heart_sprite.visible = index < get_tree().current_scene.systems.player_properties.max_hp
		index += 1
	
	

func _physics_process(delta: float) -> void:
	if get_tree().current_scene.systems.player_properties.hp < floor(heart_count / HEART_ANIMATION_FRAME_COUNT):
	...
	heart_count = min(heart_count, get_tree().current_scene.systems.player_properties.hp * HEART_ANIMATION_FRAME_COUNT)
	...
	# update gold count
	gold_count += min(abs(get_tree().current_scene.systems.player_properties.gold - gold_count), GOLD_INCREASE_RATE * delta) * sign(get_tree().current_scene.systems.player_properties.gold - gold_count)
